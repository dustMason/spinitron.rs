...
mod tests {
    ...
    fn test_parse_playlist_html_with_sample() {
        ...
        println!("Successfully parsed {} tracks from sample file", tracks.len());
        
        // Print first few tracks for verification
        for (i, track) in tracks.iter().take(5).enumerate() {
            println!("Track {}: {} - {} ({})", i + 1, track.artist, track.song, track.album);
        }
    }
    ...
    fn test_parse_malformed_html() {
        ...
        assert!(tracks.is_empty(), "Malformed HTML should result in no tracks");
    }
}