...
use models::{ShowGroup, ShowEpisode};
...
async fn main() -> Result<()> {
    ...
    println!("Scraping playlists from {} to {} (7 days)", start_date, end_date);
                    ...
                    for show in shows_to_process {
                        ...
                        match scraper::fetch_playlist(&show.url).await {
                            Ok(tracks) => {
                                ...
                                all_episodes.entry(group_key).or_insert_with(Vec::new).push(episode);
                            }
                            Err(e) => {
                                eprintln!("    ❌ Failed to fetch playlist for {}: {}", show.title, e);
                            }
                        }
                    }
                }
    ...
    for (group_key, episodes) in all_episodes {
        if let Some(_first_episode) = episodes.first() {
            ...
            println!("\n📺 Show Group: {} ({} episodes, {} total tracks)", 
                show_group.playlist_name(), show_group.episodes.len(), all_tracks.len());
            ...
            if let Some(ref mut spotify) = spotify_client {
                match spotify.create_or_update_show_playlist(&show_group).await {
                    Ok(playlist) => {
                        println!("✅ Successfully created/updated Spotify playlist: {}", playlist.name);
                        if let Some(url) = playlist.external_url {
                            println!("  🔗 Share: {}", url);
                        }
                    }
                    Err(e) => {
                        eprintln!("❌ Failed to create/update Spotify playlist for '{}': {}", show_group.playlist_name(), e);
                    }
                }
            }
        }
...
async fn output_playlist_markdown(spotify_client: &mut SpotifyClient) -> Result<()> {
    ...
    println!("\n---");
    println!("*Last updated: {}*", chrono::Utc::now().format("%Y-%m-%d %H:%M UTC"));
    println!("*Total playlists: {}*", spotify_client.get_cached_playlists().len());
